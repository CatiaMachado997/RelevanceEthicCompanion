...
class CandidateItem(BaseModel):
    ...
    source: str = Field(description="Source of the item (e.g., 'tavily', 'weaviate', 'google_calendar')")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When item was created/retrieved")
...
class ScoredItem(BaseModel):
    ...
    explanation: str = Field(description="Human-readable explanation of why this is relevant")
    score_breakdown: Dict[str, float] = Field(default_factory=dict, description="Breakdown of score components")
    ethical_flags: List[str] = Field(default_factory=list, description="Any ethical concerns raised by ESL")
    confidence: float = Field(default=1.0, ge=0.0, le=1.0, description="Confidence in the score (0-1)")
...
class RelevanceContext(BaseModel):
    ...
    active_goals: List[str] = Field(default_factory=list, description="List of active goal titles")
    upcoming_events: List[Dict[str, Any]] = Field(default_factory=list, description="Upcoming calendar events")
    ...
    user_values: List[str] = Field(default_factory=list, description="User's ethical values")
...
class RelevanceFeedback(BaseModel):
    ...
    context_snapshot: Dict[str, Any] = Field(default_factory=dict, description="Context when feedback was given")
...
class ContentSafetyCheck(BaseModel):
    ...
    violated_values: List[str] = Field(default_factory=list, description="Which user values were violated")
