...
async def test_stream_returns_200_event_stream():
    ...
    with patch(
        "orchestrator.graph.stream_langgraph", side_effect=mock_stream
...
async def test_stream_missing_message_returns_422():
    ...
    app.dependency_overrides[get_current_read_user_id] = (
...
async def test_context_builder_populates_state():
    ...
    mock_cm = MagicMock()
    mock_cm.get_user_context = AsyncMock(
...
async def test_esl_gateway_approved():
    ...
    state = {
        **base_state(),
        "proposed_content": "Here is your summary.",
        "user_context": {},
    }
...
async def test_stream_via_langgraph_path():
    ...
    with patch(
        "orchestrator.graph.stream_langgraph", side_effect=mock_langgraph
    ), patch.object(settings, "AUTH_ENFORCEMENT_ENABLED", False), patch.object(
        settings, "ENVIRONMENT", "development"
    ):
        async with AsyncClient(
