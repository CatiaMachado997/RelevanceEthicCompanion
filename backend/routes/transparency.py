...
async def get_esl_logs(
    ...
    try:
        logs = await audit_logger.get_user_logs(
            user_id=user_id, days=days, status_filter=decision_status
        )
        ...
        def _flatten_log(log) -> dict:
            ...
            return {
                ...
                "action_type": (
                    log.proposed_action.action_type.value
                    if hasattr(log.proposed_action.action_type, "value")
                    else log.proposed_action.action_type
                ),
                "decision_status": (
                    log.decision.status.value
                    if hasattr(log.decision.status, "value")
                    else log.decision.status
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching ESL logs: {str(e)}"
        )
...
async def get_esl_insights(
    ...
    try:
        ...
        if not logs:
            return {
                "user_id": str(user_id),
                "insights": ["No ESL activity in the last 7 days."],
            }
        ...
        modified_count = sum(
            1 for log in logs if log.decision.status.value == "MODIFIED"
        )
        if modified_count > 0:
            insights.append(
        ...
        approved_count = sum(
            1 for log in logs if log.decision.status.value == "APPROVED"
        )
        total = len(logs)
        approval_rate = (approved_count / total) * 100 if total > 0 else 0

        insights.append(f"✅ {approval_rate:.0f}% of actions were ethical and approved")
        ...
        return {"user_id": str(user_id), "period": "Last 7 days", "insights": insights}
