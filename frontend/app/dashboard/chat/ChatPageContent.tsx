...
/* ═══════════════════════════════════════════════════ */
export default function ChatPageContent({ conversationId: conversationIdProp }: { conversationId?: string } = {}) {
  const { user } = useAuth()
  ...
  // A Postgres-backed LangGraph interrupt survives refreshes. Recover its
  // confirmation card when reopening an existing conversation.
  useEffect(() => {
    if (!conversationId) return
    api.chat.paused(conversationId).then(payload => {
      if (!payload.paused) return
      setPausedAction({
        thread_id: conversationId,
        step: payload.step as number,
        action_index: payload.action_index as number,
        tool: payload.tool as string,
        category: payload.category as string,
        params: (payload.params as Record<string, unknown>) || {},
        reason: (payload.reason as string) || '',
        preview: payload.preview as string | undefined,
        tool_id: payload.tool_id as string | undefined,
        action_name: payload.action_name as string | undefined,
        trust_would_help: payload.trust_would_help !== false,
      })
    }).catch(() => {})
  }, [conversationId])
    ...
    try {
      ...
      const s = api.chat.stream(userMessage, {
        ...
        onToolError: (failure) => {
          setActiveTool(null)
          setMessages(prev => prev.map(m =>
            m.id === assistantId ? { ...m, toolFailure: { ...failure, request_id: requestId } } : m
          ))
        },
        ...
        onPlanPaused: (payload: unknown) => {
          ...
          setPausedAction({
            ...
            tool_id: p.tool_id as string | undefined,
            action_name: p.action_name as string | undefined,
            trust_would_help: p.trust_would_help !== false,
          })
          setReasoningByMsg(prev => {
            const cur = prev[assistantId]
                ...
                <div className="pl-9 text-sm w-full">
                  ...
                  {msg.toolFailure && (
                    <div className="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-800">
                      ...
                      {msg.toolFailure.retryable ? (
                        <button className="mt-2 font-medium underline" onClick={() => void handleSend(messages.filter(m => m.role === 'user').pop()?.content, msg.toolFailure!.request_id)}>
                          Retry request
                        </button>
                      ) : (
                        <div className="mt-2">The action was not retried automatically to avoid a duplicate. Check the destination before trying again.</div>
                      )}
                    </div>
                  )}
                  {msg.pendingConfirmation && (
                    <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
                      <div className="font-medium">{msg.pendingConfirmation.tool_name} requires confirmation</div>
                      <div className="mt-1">{msg.pendingConfirmation.preview}</div>
                      <div className="mt-2">Enable durable reasoning confirmations to approve this action without rerunning the request.</div>
                    </div>
                  )}
